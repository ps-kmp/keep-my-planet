...
CREATE TABLE IF NOT EXISTS event_state_changes (
    ...
    CONSTRAINT fk_esc_user FOREIGN KEY (changed_by) REFERENCES users(id) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS zone_state_changes (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    zone_id BIGINT NOT NULL,
    new_status TEXT NOT NULL,
    changed_by BIGINT, -- Nullable
    triggered_by_event_id BIGINT, -- Nullable
    change_time TIMESTAMP NOT NULL,
    CONSTRAINT fk_zsc_zone FOREIGN KEY (zone_id) REFERENCES zones(id) ON DELETE CASCADE,
    CONSTRAINT fk_zsc_user FOREIGN KEY (changed_by) REFERENCES users(id) ON DELETE SET NULL,
    CONSTRAINT fk_zsc_event FOREIGN KEY (triggered_by_event_id) REFERENCES events(id) ON DELETE SET NULL
);
